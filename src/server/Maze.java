...
public class Maze {
  ...
  private void generate(int cx, int cy, int max) {
    if (max > this.max) {
      this.max = max;
      this.endx = cx;
      this.endy = cy;
  ...
  public int[][] getGrid() {
    ...
    generatePowerups(grid);

    return grid;
  }

  private static void generatePowerups(int[][] grid) {
